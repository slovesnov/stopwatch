...
const char DigitalFont::digitCode[] = {
		123,
		96,
		55,
		103,
...
void DigitalFont::drawDigit(int n, cairo_t* cr) {
	...
	for (i = 0; i < 7; i++) {
		if (i == 3) {
			//left-up vertical
			a= {PDD(0,angleMargin),
				PDD(0,h-thicknessy/2-angleMargin),
				PDD(thicknessx/2,h-angleMargin/2),
				PDD(thicknessx,h-thicknessy/2-angleMargin),
				PDD(thicknessx,thicknessy+angleMargin)
			};
		}
		else if(i==0 || i==2) {
			if(i==0) {
				a= {
					PDD(thicknessx+angleMargin,thicknessy),
					PDD(angleMargin,0),
				};
			}
			else {
				a= {
					PDD(thicknessx+angleMargin,h+thicknessy/2),
					PDD(thicknessx/2+angleMargin,h),
					PDD(thicknessx+angleMargin,h-thicknessy/2),
				};
			}
